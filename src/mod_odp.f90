...
contains
  ...
  function odp_param_boot(trngl, n_boot, n_sim, dist, pgb, status) result(reserve)
    ...
    integer(c_int) :: n_pts
    real(c_double) :: X_pred(trngl%n_pred, trngl%n_cov), y_pred(trngl%n_pred)
    real(c_double) :: triangle_boot(trngl%n_dev, trngl%n_dev), betas_boot(trngl%n_cov)
    ...
    main_loop: do while (i_boot <= n_boot)
      ...
      do i = 1, trngl%n_dev
        do j = 1, trngl%n_dev + 1 - i
          if (dist == NORMAL) then
            ...
            shape = trngl%triangle_fit(i, j)**2 / (trngl%disp * trngl%triangle_fit(i, j))
            scale = (trngl%disp * trngl%triangle_fit(i, j)) / trngl%triangle_fit(i, j)
      ...
      do i_sim = 1, n_sim
        ...
        do i = 1, trngl%n_pred
          ...
          y_pred(i) = rpois(lambda)
        end do

        reserve(n_boot * (i_boot - 1) + i_sim) = sum(y_pred)
      end do
  ...
  function odp_resid_boot(trngl, n_boot, n_sim, pgb, status) result(reserve)
    ...
    main_loop: do while (i_boot <= n_boot)
      ...
      do i_sim = 1, n_sim
        ...
        reserve(n_boot * (i_boot - 1) + i_sim) = real(sum(triangle_pred), kind=c_double)
      end do
